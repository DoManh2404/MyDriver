...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderContents(breadcrumbsData, currentFolder) {
        ...
        if (!hasFolders && !hasFiles) {
            ...
            if (currentFilter === 'trash') {
                emptyTitle.textContent = "Thùng rác trống";
                emptyDesc.textContent = "Không có thư mục hoặc tệp nào trong thùng rác";
                emptyIcon.textContent = "delete_forever";
            } else if (currentFilter === 'starred') {
                emptyTitle.textContent = "Không có thư mục hoặc tệp được gắn dấu sao";
                emptyDesc.textContent = "Nhấp chuột phải vào tệp hoặc thư mục và chọn 'Gắn dấu sao'";
                emptyIcon.textContent = "star";
            } else if (searchQuery) {
                emptyTitle.textContent = "Không tìm thấy kết quả";
                emptyDesc.textContent = `Thử tìm kiếm cụm từ khác ngoài "${searchQuery}"`;
                emptyIcon.textContent = "search_off";
            } else {
                emptyTitle.textContent = "Thư mục trống";
                emptyDesc.textContent = "Kéo thả tệp hoặc sử dụng nút \"Mới\" để bắt đầu";
    ...
    function renderBreadcrumbs(crumbs, currentFolder) {
        ...
        let rootLabel = "Drive của tôi";
        if (currentFilter === 'recent') rootLabel = "Gần đây";
        else if (currentFilter === 'starred') rootLabel = "Có gắn dấu sao";
        else if (currentFilter === 'trash') rootLabel = "Thùng rác";
        
        if (searchQuery) {
            rootLabel = `Kết quả tìm kiếm cho "${searchQuery}"`;
        }
    ...
    function createFileGridItem(file) {
        ...
        if (file.visibility === 'public') {
            badgeHtml = `<span class="material-symbols-rounded visibility-badge public tooltip" data-tooltip="Công khai">public</span>`;
        } else if (file.is_shared) {
            badgeHtml = `<span class="material-symbols-rounded visibility-badge shared tooltip" data-tooltip="Đã chia sẻ với ${file.shared_count} người">group</span>`;
        }
    ...
    function createListViewTable() {
        ...
        table.innerHTML = `
            <thead>
                <tr>
                    ...
                    <th style="width: 15%">Chủ sở hữu</th>
                    <th style="width: 20%">Sửa đổi lần cuối</th>
                    <th style="width: 15%">Kích cỡ tệp</th>
                </tr>
            </thead>
            <tbody></tbody>
        `;
        ...
        activeFolders.forEach(folder => {
            ...
            tr.innerHTML = `
                ...
                <td class="list-owner-cell">tôi</td>
    ...
    function showContextMenu(e, id, type, name, isStarred) {
        ...
        if (currentFilter === 'trash') {
            ...
            if (isStarred) {
                starText.textContent = "Xóa dấu sao";
                starIcon.textContent = "star_rate";
                starIcon.style.color = "#f29900";
            } else {
                starText.textContent = "Gắn dấu sao";
    ...
    function updateUploadWidgetHeaderTitle() {
        ...
        if (items.length === 0) {
            uploadWidgetTitle.textContent = "Không có tệp tải lên";
        } else if (successes === items.length) {
            uploadWidgetTitle.textContent = `Đã tải lên thành công ${successes}/${items.length} tệp`;
        } else {
            uploadWidgetTitle.textContent = `Đang tải lên ${items.length - successes} tệp...`;
        }
    }
    ...
    function fetchStorageUsage() {
        fetch('/api/storage/')
            ...
            .then(data => {
                ...
                storageText.textContent = `Đã dùng ${data.readable} trong tổng số 15 GB`;
            })
            .catch(err => {
    ...
    function renameItem(newName) {
        ...
        fetch('/api/item/rename/', {
            ...
            .catch(err => {
                alert(err.message || "Không thể đổi tên.");
            });
    }
    ...
    function fetchSharedUsers(fileId) {
        sharedUsersList.innerHTML = '<div style="padding:10px; font-size:12px; color:var(--text-secondary); text-align:center;">Đang tải danh sách người dùng...</div>';
        ...
        fetch(`/api/file/shared-users/?file_id=${fileId}`)
            ...
            .then(data => {
                ...
                if (users.length === 0) {
                    sharedUsersList.innerHTML = '<div style="padding:16px; font-size:12px; color:var(--text-secondary); text-align:center;">Chưa chia sẻ với bất kỳ ai</div>';
                } else {
                    users.forEach(user => {
                        const itemDiv = document.createElement('div');
                        ...
                        itemDiv.innerHTML = `
                            <div class="shared-user-info">
                                ...
                                <span class="shared-user-email">${escapeHTML(user.email || 'không có email')}</span>
                            </div>
                            <button class="remove-share-btn" data-user-id="${user.id}">Thu hồi</button>
                        `;
            ...
            .catch(err => {
                ...
                sharedUsersList.innerHTML = '<div style="padding:10px; font-size:12px; color:var(--text-danger); text-align:center;">Không thể tải danh sách</div>';
            });
    }
    ...
    function shareFileVisibility(visibility) {
        ...
        fetch('/api/file/share/', {
            ...
            .catch(err => {
                ...
                alert("Không thể cập nhật quyền truy cập chung.");
            });
    }
    ...
    function addShareUser(username) {
        ...
        fetch('/api/file/share/', {
            ...
            .catch(err => {
                ...
                shareErrorText.textContent = "Không kết nối được máy chủ.";
                shareErrorMessage.style.display = 'flex';
            });
    }
    ...
    function removeShareUser(userId) {
        ...
        fetch('/api/file/share/remove/', {
            ...
            .catch(err => {
                ...
                alert("Không thể thu hồi quyền truy cập.");
            });
    }
    ...
    function bindEvents() {
        ...
        confirmFolderBtn.addEventListener('click', () => {
            ...
            createFolder(name || "Thư mục chưa đặt tên");
        });
        ...
        layoutToggleBtn.addEventListener('click', () => {
            ...
            if (currentLayout === 'grid') {
                ...
                layoutToggleBtn.setAttribute('data-tooltip', "Dạng lưới");
            } else {
                currentLayout = 'grid';
                toggleIcon.textContent = "view_list";
                layoutToggleBtn.setAttribute('data-tooltip', "Dạng danh sách");
            }
            fetchContents();
        });
        ...
        copyLinkBtn.addEventListener('click', () => {
            ...
            copyLinkBtn.textContent = "Đã chép!";
            setTimeout(() => {
